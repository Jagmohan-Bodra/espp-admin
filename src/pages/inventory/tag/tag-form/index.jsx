...
import TabTagSEO from './TabTagSEO';
...
const cssClass = 'inventory-tag-form';

const TagForm = (props) => {
  const [tab, setTab] = useState('TAG_DETAIL');
  ...
  const renderTabContent = (tab, data) => {
    switch (tab) {
      case 'TAG_DETAIL':
        return (
          <TabTagDetail data={data} onSave={onSave} isUpdate={props.isUpdate} />
        );
      case 'TAG_SEO':
        return (
          <TabTagSEO data={data} onSave={onSave} isUpdate={props.isUpdate} />
        );
    }
  };
...
const getBreadcrumb = (id, data) => {
  ...
  return [
    {name: 'Tags', link: PATH.INVENTORY_TAG},
    {name: nameBread, link: '#'},
  ];
};
